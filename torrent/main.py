...
async def udp():
    ...
    loop = asyncio.get_running_loop()
        ...
        # Create a datagram endpoint and bind it to a local address
    transport, protocol = await loop.create_datagram_endpoint(
        lambda: MyDatagramProtocol(), remote_addr=('98.156.26.225', 44957)
    )

    try:
        await asyncio.sleep(3600)  # Keep the server running for an hour
    
        utp = MicroTransportProtocol(protocol)
        utp.connection_made(transport)
        print(utp)
    finally:
        transport.close()
