...
pub fn parse_response(scheme: Scheme, res: &mut Vec<u8>) -> Result<&mut [u8], UttdError> {
    match scheme {
        Scheme::UDP => parse_response_udp(res),
        _ => parse_response_tcp(res),
    }
}
...
pub fn parse_response_tcp(res: &mut Vec<u8>) -> Result<&mut [u8], UttdError> {
    let response_code =
...
mod test {
    ...
    fn parse_ok() {
        ...
        let res = parse_response(crate::url::Scheme::HTTP, &mut value);
